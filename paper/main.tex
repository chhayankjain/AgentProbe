...
\corres{Correspondence: chhayank44@gmail.com}
...
\begin{figure}[htbp]
  ...
  \caption{Summary dashboard showing cross-framework reliability comparison: (a)~overall failure rate, (b)~recovery rate under injection conditions, (c)~mean latency, and (d)~P99 latency by task type. Based on 1{,}050 runs across LangGraph, LangChain, and AutoGen with Llama~3.1 via Ollama.}\label{fig:architecture}
\end{figure}
...
A total of 1{,}050 experiment runs were completed across all conditions. Full results data and analysis scripts are available in the project repository.

\subsection{Baseline Performance}

Under baseline conditions (no fault injection), all three frameworks achieve a 0\% failure rate [95\%~CI: 0.0--4.1\%] across all task types ($n = 90$ per framework for LangGraph/LangChain, $n = 30$ for AutoGen). Mean latency varies by task: tool use is fastest (LangGraph: 5.9\,s, LangChain: 6.0\,s), followed by multi-agent (8.4\,s, 10.7\,s) and RAG (10.3\,s, 11.4\,s). AutoGen exhibits high variance, with mean latency ranging from 4.8\,s for tool use to 22.7\,s for multi-agent tasks.
...
Mean latency is comparable across frameworks (AutoGen: 7{,}663\,ms, LangChain: 7{,}492\,ms, LangGraph: 7{,}702\,ms), but the omnibus Kruskal--Wallis test reveals a significant difference in latency distributions ($H = 11.65$, $p = 0.003$). This is driven primarily by AutoGen's bimodal latency profile: near-zero P99 under injection conditions (where tool bypass produces fast but ungrounded responses) versus very high P99 under baseline conditions (149\,s for multi-agent tasks, where the conversation protocol incurs overhead without tool execution to anchor the interaction).
...
\authorcontributions{Conceptualization, methodology, software, investigation, writing---original draft preparation, C.J.}
